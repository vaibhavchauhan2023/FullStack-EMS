...
import { dummyAttendanceData, dummyProfileData } from "../assets/assets"
import { CalendarIcon, ChevronRightIcon, CrossIcon, DollarSign, DollarSignIcon, FileTextIcon, LayoutGridIcon, Loader2, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, UsersIcon, XIcon } from "lucide-react"
...
const Sidebar = () => {
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] font-medium text-slate-500">Management System</p>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
                            ...
                            <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-indigo-300" : "text-slate-400"}`} />
  ...
  return (
    <>
        ...
        <button onClick={()=>setMobileOpen(true)} className="lg:hidden fixed top-4 left-4 z-50 bg-slate-900 text-white rounded-lg shadow-lg border border-white/10">
            <MenuIcon size={20}/>
        </button>
        ...
        {/*Desktop - sidebar*/}
        <aside className="hidden lg:flex flex-col h-full w-65 bg-linear-to-b from bg-slate-900 via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4">
            {sidebarContent}
        </aside>

        {/*Mobile - sidebar*/}
        <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from bg-slate-900 via-slate-900 to-slate-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0":"-translate-x-0"}`}>
            {sidebarContent}
        </aside>
