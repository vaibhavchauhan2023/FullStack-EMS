...
const AttendanceStats = ({history}) => {
    const totalPresent = history.filter((h)=> h.status === "PRESENT" || "LATE").length;
        ...
        {stats.map((s)=>(
            <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group'>
                ...
                <div className='p-3 bg-slate-100 rounded-lg group-hover:bg-indigo-50 transition-colors duration-200'>
                    {<s.icon className='w-h h-5 text-slate-600 group-hover:text-indigo-600 transition-colors duration-200' />}
                </div>
                <div>
                    <p className='text-sm text-slate-500'>{s.label}</p>
                    <h2 className='text-2xl font-medium text-slate-900 tracking-tight'>{s.value}</h2>
                </div>
            </div>
        ))}
